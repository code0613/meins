...
`navigator.share` 는 **시스템 공유 시트를 여는 것**이지 네트워크로 보내는 것이 아닙니다.
어디로 보낼지는 사용자가 그 자리에서 고르고, iOS에서는 「이미지 저장」이 이 시트 안에 있습니다.
인앱 브라우저는 `<a download>` 를 무시하는 경우가 많아 모바일에서 결과물을 가져가는 주 경로입니다.

`fetch` 로 오해해서 지우지 마세요. 다만 이걸 근거로 다른 외부 통신을 추가하지도 마세요.
