...
주민등록증 사본이나 영수증을 보낼 일이 생기면 보통 온라인 편집 도구를 찾게 됩니다.
그런데 가리려던 그 사진을 남의 서버에 올리는 걸로 시작하죠. 그 단계를 없애고 싶었습니다.
...
| | |
|---|---|
| 통신 코드 | `fetch`, `XHR`, `WebSocket` 이 한 줄도 없습니다 |
| 저장소 | `localStorage`, `IndexedDB`, 쿠키를 쓰지 않습니다. 탭을 닫으면 남는 게 없습니다 |
| 외부 스크립트 | 없습니다. 분석 도구도 CDN도 붙이지 않았습니다 |
| CSP | 배포본에 `connect-src 'none'`. 브라우저가 외부 요청을 막습니다 |

마지막 줄이 핵심입니다. "안 보낸다"는 말만으로는 믿기 어려우니,
**실수로 코드에 통신이 섞여도 브라우저가 차단하도록** 헤더를 걸어뒀습니다.
