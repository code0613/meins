...
<https://meins-image-blur.vercel.app>
...
apps/image-blur/
├─ vercel.json              배포 설정
├─ vite.config.ts           CSP는 운영 빌드에만 주입된다
└─ src/
   ├─ features/Masking/     마스킹 기능 일체
   ├─ features/Donation/    후원 모달
   ├─ features/Licenses/    오픈소스 고지
   └─ router/ · style/
```

React 19 · TypeScript · Vite 5 · MUI v5.
색은 `src/style/global.ts`의 CSS 변수가 기준입니다.

## 개발

```bash
yarn install                # 저장소 루트에서
yarn image-blur dev
yarn image-blur build
yarn image-blur lint
```

## 라이선스 고지

쓰고 있는 오픈소스 목록은 `/licenses` 경로에 있습니다. 화면에 링크를 두지는 않았습니다.
의존성이 바뀌면 저장소 루트에서 아래를 다시 돌리세요.
...
네트워크 통신 금지와 문구 정합성 같은, 이 앱에만 걸리는 제약이 정리돼 있습니다.
